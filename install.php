...
$plugin_name = 'cfg_archive';

// Set a title
$window['title'] = "Network Device Config Archive Install";
...
//
$pmodules = array();
//$pmodules['build_tinydns_conf']['desc'] = 'Build TinyDNS server configuration';
//$pmodules['build_tinydns_conf']['file'] = "..{$plugindir}/{$plugin_name}.inc.php";
...
$window['html'] .= "&nbsp;&nbsp;&nbsp;&nbsp;<br><center><b><img src='{$images}/silk/error.png' border='0'>You must first execute the config.sh script<br> located in {$installdir}</b></center><br>";

$window['html'] .= "<br><b>Checking required files:</b><br>";

$reqfile = $onainstalldir.'/bin/cfg_archive';
if (file_exists($reqfile)) {
    $window['html'] .= "&nbsp;&nbsp;&nbsp;&nbsp;<img src='{$images}/silk/accept.png' border='0'> {$reqfile}<br>";
} else {
    $stat++;
    $window['html'] .= "&nbsp;&nbsp;&nbsp;&nbsp;<img src='{$images}/silk/error.png' border='0'> <font color=\"red\">ERROR=> Install files:</font><br>";
    // Provide some information to the user on how to fix the problem manually
    $window['html'] .= "<font color=\"orange\">Execute shell command:<br>ln -s {$installdir}/bin/cfg_archive {$reqfile}</font><br><br>";
}

$reqfile = $onainstalldir.'/bin/cfg_archive_procspool';
if (file_exists($reqfile)) {
    $window['html'] .= "&nbsp;&nbsp;&nbsp;&nbsp;<img src='{$images}/silk/accept.png' border='0'> {$reqfile}<br>";
} else {
    $stat++;
    $window['html'] .= "&nbsp;&nbsp;&nbsp;&nbsp;<img src='{$images}/silk/error.png' border='0'> <font color=\"red\">ERROR=> Install files:</font><br>";
    // Provide some information to the user on how to fix the problem manually
    $window['html'] .= "<font color=\"orange\">Execute shell command:<br>ln -s {$installdir}/bin/cfg_archive_procspool {$reqfile}</font><br><br>";
}
...
if (!$stat) {
    $window['html'] .= @rename(__FILE__, __FILE__.'.completed') ? "&nbsp;&nbsp;&nbsp;&nbsp;<img src='{$images}/silk/accept.png' border='0'>Moved install files.<br><br><center><b>Install complete.<br>You should read the documentation located <a href='https://github.com/opennetadmin/{$plugin_name}/wiki'>HERE</a> for further install instructions.</b><br><br><a onclick=\"removeElement('{$window_name}');\">CLOSE WINDOW</a><br></center>" : "<br>&nbsp;&nbsp;&nbsp;&nbsp;<img src='{$images}/silk/error.png' border='0'> <font color=\"red\">ERROR=> Unable to rename install file, do it manually then close this window:</font><br><br><font color=\"orange\">mv {$installfile} {$installfile}.completed</font><br><br>";
} else {
