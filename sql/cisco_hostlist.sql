-- Selects the FQDN of all cisco devices for archival
--
-- LONG_DESC: 
--	Selects all the cisco devices from the database.  Used to
                ...
                select id from devices where device_type_id in (
                        select id from device_types where model_id in (
                                select id from models where manufacturer_id in (
                                        select id from manufacturers where name = "cisco"
                                )
                        )
                )
        )
